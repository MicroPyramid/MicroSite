...
class Views_test(TestCase):
    ...
    def test_views(self):
        ...
        #Login code test
        response = self.client.post('/portal/', {'email': 'micro@micropyramid.com', 'password': 'JNZUNA61EC'})
...
class test_portal_admin(TestCase):
    ...
    def setUp(self):
        self.client = Client()
        self.user = User.objects.create_superuser('microtest', 'mp@mp.com', 'mp')
        self.employee = User.objects.create_user('testemployee', "test@gmail.com", 'pwd')

    def test_user_index(self):
        # Testcase for forgot password with wrong input
        response = self.client.post('/portal/forgot-password/', {'email': 'dfdfd'})
        self.assertEqual(response.status_code,200)
        self.assertTrue('"message": "Entered Email id is incorrect."' in response.content)

        # Testcase for forgot password with correct input
        response = self.client.post('/portal/forgot-password/', {'email': 'test@gmail.com'})
        self.assertEqual(response.status_code,200)
        self.assertTrue('"message": "Password has been sent to your email sucessfully."' in response.content)

        # Testcase for user login with wrong input
        response = self.client.post('/portal/', {'email': 'dfdfd', 'password': 'mpdfdf'})
        self.assertEqual(response.status_code,200)
        self.assertTrue('"message": "The username and password are incorrect."' in response.content)

        # Testcase for user login with correct input
        response = self.client.post('/portal/', {'email': 'mp@mp.com', 'password': 'mp'})
        self.assertEqual(response.status_code,200)
...
class user_test(TestCase):
    ...
    def test_views_user(self):
        ...
        response = self.client.post('/portal/users/new/', {'first_name': 'Micro', 'last_name':'Pyramid', 'user_roles':'Admin', 'state': 'MP', 'city': 'HYD', 'area': 'KPHB'})
        self.assertEqual(response.status_code,200)
